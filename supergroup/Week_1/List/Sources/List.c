            ...
            if (c <= 'F' && c >= 'A')
            {
                ...
                case 'A':
                    ...
                    while (scanf("%d", &n) != 1 || n > 10000)
                    {
                        ...
                        printf("输入有误,请输入小于10000的整数:");
                        fflush(stdin);
                    }
                    detect_illegal_input();
                    
                    for (int i = 0; i < n; i++)
                    {
                        printf("请输入第%d个节点的值(输入整数):", i + 1);

                        while (scanf("%d", &data[i]) != 1)
                        {

                            printf("输入有误,请重新输入第%d个节点数据:", i + 1);
                            fflush(stdin);
                        }
                        detect_illegal_input();
                    }
        ...
        if (kbhit())
        {
            ...
            if (c <= 'F' && c >= 'A')
            {
                ...
                case 'B':
                    ...
                    {
                        q_dNode = head_dList;
                        while (q_dNode->data != p_data)
                            q_dNode = q_dNode->next;
                    }
                    else
                    {
                        printf("未找到该节点,插入失败\n");
...
int detect_illegal_input(){
    ...
    // int c;
    // if(scanf("%d", &c)!=1){
    //     printf("检测到部分非法输入,先只取输入最前端的合法数据\n");
    //     fflush(stdin);
    //     return 0;
    // }
    // else return 1;
